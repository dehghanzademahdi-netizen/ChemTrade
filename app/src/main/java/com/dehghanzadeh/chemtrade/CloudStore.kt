...
object CloudStore {
    ...
    private suspend fun request(method: String, path: String, body: String? = null): Response = withContext(Dispatchers.IO) {
        runCatching {
            val c = (URL("${baseUrl()}/$path.json").openConnection() as HttpURLConnection).apply {
                requestMethod = method; connectTimeout = 15000; readTimeout = 20000; useCaches = false
            ...
            c.disconnect(); Response(code, result)
    ...
    suspend fun saveUser(prefs: SharedPreferences, userJson: JSONObject): Boolean {
        ...
        val keys = userJson.keys(); while (keys.hasNext()) { val k = keys.next(); merged.put(k, userJson.opt(k)) }
        val put = request("PUT", path, merged.toString()); if (put.code !in 200..299) return false
        val verify = request("GET", path); if (verify.code !in 200..299) return false
        ...
        if (usersResponse.code !in 200..299) return false
        val users = array(usersResponse.body)
        var found = false
        ...
        val usersPut = request("PUT", "$ROOT/$USERS", users.toString())
        return usersPut.code in 200..299
    }

    suspend fun loadUser(prefs: SharedPreferences, phone: String): JSONObject? {
        val normalized = phone.filter { it.isDigit() }; if (normalized.isBlank()) return null
        ...
        val usersResponse = request("GET", "$ROOT/$USERS")
        if (usersResponse.code !in 200..299) return null
        val users = array(usersResponse.body)
        for (i in 0 until users.length()) {
            val u = users.optJSONObject(i) ?: continue
    ...
    suspend fun push(prefs: SharedPreferences): Boolean {
        val localUsers = array(prefs.getString(USERS, "[]")); val localOffers = array(prefs.getString(OFFERS, "[]"))
        val remoteResponse = request("GET", ROOT)
        if (remoteResponse.code !in 200..299) return false
        val remote = remoteResponse.body?.let { runCatching { JSONObject(it) }.getOrNull() } ?: JSONObject()
        val remoteUsers = array(remote.optString(USERS)); val remoteOffers = array(remote.optString(OFFERS))
        ...
        for (i in 0 until mergedUsers.length()) { val u = mergedUsers.optJSONObject(i) ?: continue; val p = u.optString("phone").filter { it.isDigit() }; if (p.isNotBlank()) usersByPhone.put(p, u) }
        ...
        return request("PUT", ROOT, payload.toString()).code in 200..299
    }

    suspend fun pull(prefs: SharedPreferences): Boolean {
        val response = request("GET", ROOT); if (response.code !in 200..299) return false
        ...
        prefs.edit().putString(USERS, users.toString()).putString(OFFERS, offers.toString()).apply(); return true
    }
